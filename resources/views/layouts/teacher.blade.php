...
<head>
    ...
    <title>@yield('title', 'Teacher') - Interacts</title>
                ...
                <nav class="nav flex-column px-3">
                    ...
                    @if(isset($classrooms) && $classrooms->count() > 0)
                        @foreach($classrooms as $classroom)
                            <a class="nav-link small" href="{{ route('teacher.appointments', $classroom->classroom_id) }}">
                                <i class="bi bi-door-open me-2"></i>{{ Str::limit($classroom->classroom_name, 20) }}
                            </a>
                        @endforeach
                    @else
                        <div class="text-white-50 small px-3">No classrooms assigned</div>
                    @endif
